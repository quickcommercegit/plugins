...
$title      = $settings['checkout_billing_form_title_text'];
$form_label = $settings['checkout_billing_form_label_title_text'];

if ( sizeof( $checkout->checkout_fields ) > 0 ) : ?>
	<div class="woocommerce-billing-fields">
		<?php if ( ! empty( $title ) ) : ?>
			<h3><?php esc_html_e( $title, 'jet-woo-builder' ); ?></h3>
		<?php endif; ?>
			...
			<?php if ( ! $checkout->is_registration_required() ) : ?>
				<p class="form-row form-row-wide create-account">
					<label class="woocommerce-form__label woocommerce-form__label-for-checkbox checkbox">
						...
						<span><?php echo ! empty( $form_label ) ? esc_html__( $form_label, 'jet-woo-builder' ) : esc_html__( 'Create an account?', 'woocommerce' ); ?></span>
					</label>
				</p>
			<?php endif; ?>
