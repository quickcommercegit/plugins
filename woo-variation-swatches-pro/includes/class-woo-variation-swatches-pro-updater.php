...
if ( ! class_exists( 'Woo_Variation_Swatches_Pro_Updater' ) ):
	final class Woo_Variation_Swatches_Pro_Updater {
		...
		public function __construct( $plugin_file, $product_id, $license_key = '', $args = array() ) {
			...
			// $this->get_plugin_update_cache();
			// wp_clean_plugins_cache()


			// print_r($this->get_plugin_update_cache()); die;


			$this->plugin_id   = trim( str_ireplace( array(
				'http://',
				'https://'
			), '', $this->plugin_homepage ), '/' );
			...
			add_action( 'admin_action_wvs_check_update', array( $this, 'force_update_check' ) );
		...
		public function check_for_update_link( $links, $file ) {
			...
			if ( $file == $this->plugin_slug && current_user_can( 'update_plugins' ) ) {
				...
				$update_check_url = esc_url( add_query_arg( array( 'action' => 'wvs_check_update' ), admin_url( 'plugins.php' ) ) );

				$row_meta['wvs_check_update'] = '<a href="' . $update_check_url . '" title="' . esc_attr( esc_html__( 'Check Update', 'woo-variation-swatches' ) ) . '">' . esc_html__( 'Check Update', 'woo-variation-swatches' ) . '</a>';
		...
		public function check_update( $transient ) {
			...
			$item = array(
				...
				'icons'         => array(
					...
					// 'svg' => sprintf( 'https://ps.w.org/%s/assets/icon.svg', $this->free_plugin_slug ),
				),
				'banners'       => array(
					'2x' => sprintf( 'https://ps.w.org/%s/assets/banner-1544x500.png', $this->free_plugin_slug ),
					'1x' => sprintf( 'https://ps.w.org/%s/assets/banner-772x250.png', $this->free_plugin_slug ),
				),
				'banners_rtl'   => array(),
				'tested'        => $this->tested,
