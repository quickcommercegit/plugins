	...
	final class Woo_Variation_Swatches_Pro {
		...
		public function includes() {
			if ( $this->is_wvs_active() ) {
				require_once $this->include_path( 'class-woo-variation-swatches-pro-updater.php' );
		...
		public function hooks() {
			...
			if ( $this->is_wvs_active() ) {
				...
				add_action( 'in_plugin_update_message-' . $this->basename(), array(
					$this,
					'plugin_update_message_row'
				), 10, 2 );
			} else {
		...
		public function updater() {
			if ( class_exists( 'Woo_Variation_Swatches_Pro_Updater' ) ) {
				$license_key = trim( woo_variation_swatches()->get_option( 'license_key' ) );
				new Woo_Variation_Swatches_Pro_Updater( __FILE__, 113, $license_key );
			}
		}
		...
		public function enqueue_scripts() {
			...
			// Disable All scripts
			if ( apply_filters( 'disable_woo_variation_swatches_pro', false ) ) {
				return;
			}
