...
if ( ! class_exists( 'YITH_WCAF' ) ) {
	...
	class YITH_WCAF {
		...
		public function __construct() {
			...
			add_filter( 'woocommerce_get_breadcrumb', array( $this, 'filter_page_breadcrumb' ), 10 );
		 ...
		 */
		public function filter_page_breadcrumb( $crumbs ) {
			global $post;

			if ( ! $this->is_dashboard_url() ) {
				return $crumbs;
			}
			...
			if ( $current_endpoint ) {
				...
				if ( $post ) {
					...
					$crumbs[] = array(
						$post->post_title,
						$this->get_affiliate_dashboard_url()
					);
				}
