...
if ( ! class_exists( 'YITH_WCAF_Admin' ) ) {
	...
	class YITH_WCAF_Admin {
		...
		public function __construct() {
			...
			add_action( 'admin_init', array( $this, 'register_current_instance' ) );
		...
		public function admin_notices() {
			...
			if ( in_array( $screen->id, $this->get_screen_ids(), true ) ) {
				...
				if ( isset( $_GET['commissions_paid'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					if ( 1 === intval( $_GET['commissions_paid'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				if ( isset( $_GET['affiliate_added'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					if ( 1 === intval( $_GET['affiliate_added'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				if ( isset( $_GET['commission_status_change'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					if ( 1 === intval( $_GET['commission_status_change'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				if ( isset( $_GET['commission_deleted'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					if ( 1 === intval( $_GET['commission_deleted'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				if ( isset( $_GET['processed_dangling'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					if ( 1 === intval( $_GET['processed_dangling'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		 ...
		 * This is used to perform safety checks later in the plugin; for eaxample, before performing a payment
		 ...
		 * @return void
		 */
		public function register_current_instance() {
			$registered_instance = get_option( 'yith_wcaf_instance', '' );

			if ( $registered_instance ) {
				return;
			}

			$current_instance = str_replace( array( 'https://', 'http://', 'www.' ), '', get_site_url() );

			update_option( 'yith_wcaf_instance', $current_instance );
		}
