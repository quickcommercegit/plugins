...
class Admin_Menu_Module extends Base_Module {
	...
	public function setup() {
		...
		add_action( 'admin_notices', array( self::get_instance(), 'non_blueprint_notice' ) );
	...
	public function non_blueprint_notice() {
		...
		$description  = __( '<strong>Caution:</strong> If the Admin Menu Editor is configured on a subsite, the blueprint settings for this feature will no longer be inherited.', 'ultimatedashboard' ) . '<br>';
		$description .= __( 'To inherit the blueprint configuration again for the Admin Menu Editor, please <strong>reset all menus</strong> (button below).', 'ultimatedashboard' ) . '<br>';
		$description  = '<p>' . $description . '</p>';

		printf( '<div class="notice notice-warning is-dismissible">%1s</div>', $description );
	...
	public function super_admin_notice() {
		...
		$description = $description . ' ' . __( 'Changes made to the <strong>Administrator</strong> role will not affect super admins. Super admins will always see the full admin menu for maximum control.', 'ultimatedashboard' );
		$description = '<p>' . $description . '</p>';
