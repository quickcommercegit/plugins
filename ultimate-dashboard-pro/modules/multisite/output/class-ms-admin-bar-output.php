...
class Ms_Admin_Bar_Output extends Base_Output {
	...
	public function ms_get_saved_menu( $saved_menu ) {
		...
		$switch_blog = $ms_helper->needs_to_switch_blog() ? true : false;
		$admin_bar   = get_option( 'udb_admin_bar', array() );

		if ( $switch_blog && empty( $admin_bar ) ) {
			global $blueprint;
	...
	public function menu_output( $admin_bar ) {
		...
		$parsed_menu   = $output_module->get_parsed_menu( $admin_bar, true );
		...
		$output_module->generate_nodes( $admin_bar, $parsed_menu, true );
